...
Public Class Frm_QC_Report1
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        '         Select PS_Date, PS_History, Sum(Convert(Decimal,PS_Total)) AS F1 From PC_STOCK_Er with(NOLOCK), PC_STOCK with(NOLOCK) Where   PC_STOCK.PS_Code = PC_STOCK_Er.PS_Code And   PS_Date + PS_Time >= '2019-04-0100:00' And PS_Date + PS_Time <= '2019-04-1123:59'  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date
        StrSQL = "Select PS_Date, PS_History, Sum(Convert(Decimal,PS_Total)) AS F1 From PC_STOCK_Er with(NOLOCK), PC_STOCK with(NOLOCK) Where  PC_STOCK.PS_Code = PC_STOCK_Er.PS_Code And  PS_Date + PS_Time >= '" & S_Day.Text & S_Time.Text & "' And PS_Date + PS_Time <= '" & E_Day.Text & E_Time.Text & "'  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date "
        'Select PS_Date, PS_History, Sum(Convert(Decimal,PS_Total)) AS F1 From PC_STOCK_Er with(NOLOCK), PC_STOCK with(NOLOCK) Where   PC_STOCK.PS_Code = PC_STOCK_Er.PS_Code And  And PS_Date + PS_Time >= '2019-04-0100:00' And PS_Date + PS_Time <= '2019-04-1123:59'  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date 
    ...
    Private Sub Chart_Display()
        ...
        StrSQL = "Select Sum(Convert(Decimal,PS_Total)) AS F1 
                    From PC_STOCK_Er with(NOLOCK), PC_STOCK with(NOLOCK) 
                   Where   PC_STOCK.PS_Code = PC_STOCK_Er.PS_Code  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date"

        Re_Count = DB_Select(DBT)

        '불량 유형 개수를 센다

        StrSQL = "SELECT BASE_NAME FROM SI_BASE_CODE WHERE BASE_SEL = '불량유형'"
        Re_Count = DB_Select(DBT)
        ...
        For i = 0 To 불량유형개수 ' ~ 4
            StrSQL = "Select PS_HISTORY, Sum(Convert(Decimal,PS_Total)) AS F1 
                    From PC_STOCK_Er with(NOLOCK), PC_STOCK with(NOLOCK) 
                   Where PC_STOCK.PS_Code = PC_STOCK_Er.PS_Code  And Len(PS_Total) > 0 and ps_history='" & 불량유형이름(i) & "' GROUP BY PS_Date, PS_History  Order By PS_Date"
