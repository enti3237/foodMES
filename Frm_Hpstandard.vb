...
Public Class Frm_Hpstandard
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(3).HeaderText = "문서이름"
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        Grid_Main.RowCount = 0
        If Search_Combo_Name.Text = "전체" Then
            StrSQL = "Select HP_DATE, HP_TIME, HP_NAME, HP_BIGO FROM HP_LIST WITH(NOLOCK)  Where  HP_Date + HP_Time >= '" & S_Day.Text & S_Time.Text & "' And HP_Date + HP_Time <= '" & E_Day.Text & E_Time.Text & "' Order By HP_Date, HP_Time"
        Else
            StrSQL = "Select HP_DATE, HP_TIME, HP_NAME, HP_BIGO FROM HP_LIST WITH(NOLOCK)  Where  HP_Date + HP_Time >= '" & S_Day.Text & S_Time.Text & "' And HP_Date + HP_Time <= '" & E_Day.Text & E_Time.Text & "' Order By HP_Date, HP_Time"
        End If
        Re_Count = DB_Select(DBT)
