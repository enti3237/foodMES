Public Class Frm_Haccp3
    ...
    Public Function Process_Line_Grid_Setup() As Boolean
        ...
        Process_Line_Grid.Columns(1).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        Process_Line_Grid.Columns(2).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter

        Process_Line_Grid.RowHeadersWidth = 40
        Process_Line_Grid.Columns(0).Width = 100
        Process_Line_Grid.Columns(1).Width = 300
        ...
        Process_Line_Grid.Columns(0).Visible = False
        Process_Line_Grid.Columns(1).HeaderText = "문서명"
        Process_Line_Grid.Columns(2).HeaderText = "주기(일)"
        Process_Line_Grid.Columns(3).HeaderText = "기준서버전"
    ...
    Public Function Process_Line_Grid_Display() As Boolean
        ...
        StrSQL = "Select * FROM hp_standard_list with(NOLOCK) Order By ST_CODE"
    ...
    Private Sub Save_Com_Click(sender As Object, e As EventArgs) Handles Save_Com.Click
        ...
        For i = 0 To Process_Line_Grid.Rows.Count - 1
            If Process_Line_Grid.Rows(i).HeaderCell.Value = "*" Then
                ...
                StrSQL = StrSQL & "UPDATE hp_standard_list Set
                                     ST_Doc = '" & Process_Line_Grid.Item(1, i).Value & "',
                                     ST_Date = '" & Process_Line_Grid.Item(2, i).Value & "',
                                     ST_STANDARD = '" & Process_Line_Grid.Item(3, i).Value & "',
    ...
    Private Sub Insert_Com_Click(sender As Object, e As EventArgs) Handles Insert_Com.Click
        ...
        Dim DBT As New DataTable
        Dim Db_Sun As Long
        StrSQL = "Select ST_CODE FROM hp_standard_list with(NOLOCK) Order By Convert(Decimal,ST_CODE) Desc "
        Re_Count = DB_Select(DBT)
        If Re_Count = 0 Then
            Db_Sun = 1
        Else
        ...
        '추가한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Insert INTO hp_standard_list (ST_CODE)  Values ('" & Db_Sun & "')"
        Re_Count = DB_Execute()
        Process_Line_Grid_Display()
    ...
    Private Sub Delete_Com_Click(sender As Object, e As EventArgs) Handles Delete_Com.Click
        ...
        End If
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "DELETE hp_standard_list where ST_CODE = '" & Process_Line_Grid.Item(0, Process_Line_Grid.CurrentCell.RowIndex).Value & "'"
        Re_Count = DB_Execute()
        Process_Line_Grid_Display()
        MsgBox("삭제 되었습니다.")
    End Sub
