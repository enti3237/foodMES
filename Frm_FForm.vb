Public Class Frm_FForm
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(1).HeaderText = "금형이름"
    ...
    Private Sub Insert_Com_Click(sender As Object, e As EventArgs) Handles Insert_Com.Click
        ...
        StrSQL = "Select FF_Code From FF_Conf with(NOLOCK) Where FF_Date Like  '" & My_Date & "%' Order By FF_Code Desc "
        Re_Count = DB_Select(DBT)
        If Re_Count = 0 Then
            JP_Code = JP_Code & "001"
        Else
        ...
        StrSQL = StrSQL & "Insert into FF_Conf (FF_Code, FF_Date, FF_Time, FF_Name)  Values ('FF" & JP_Code & "', '" & My_Date & "','" & My_Time & "','" & Frm_Main.Text_Man_Name.Text & "')"
        Re_Count = DB_Execute()
        Grid_Code_Display()
    ...
    Public Function Grid_Code_Display() As Boolean
        ...
        Select Case Search_Combo.Text
            Case "코드"
                StrSQL = "Select FF_Code, FF_Date, FF_Name From FF_Conf with(NOLOCK) Where FF_Code Like '" & Search_Text.Text & "%'  Order  By FF_Code DESC"
            Case "날짜"
                StrSQL = "Select FF_Code, FF_Date, FF_Name From FF_Conf with(NOLOCK) Where FF_Date Like '" & Search_Text.Text & "%'  Or FF_Date Is Null Order  By FF_Date Desc"
            Case "담당"
                StrSQL = "Select FF_Code, FF_Date, FF_Name From FF_Conf with(NOLOCK) Where FF_Name Like '" & Search_Text.Text & "%'   Order  By FF_Name Desc"
        End Select
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Public Function Grid_Info_Display(Code As String) As Boolean
        ...
        Dim DBT As New DataTable
        Dim i As Integer
        StrSQL = "Select * From FF_Conf With(NOLOCK) Where FF_Code = '" & Code & "'"
        Re_Count = DB_Select(DBT)
        If Re_Count > 0 Then
    ...
    Public Function Grid_Main_Display(PL_Code As String) As Boolean
        ...
        Grid_Display_Ch = False
        Grid_Main.RowCount = 0
        StrSQL = "Select FF_Sun, FF_Name, FF_Total, FF_Bigo  From FF_Conf_List with(NOLOCK)  Where FF_Code = '" & PL_Code & "' Order By Convert(Decimal,FF_Sun)"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Private Sub Insert__Main_Click(sender As Object, e As EventArgs) Handles Insert__Main.Click
        ...
        Dim DBT As New DataTable
        Dim Db_Sun As Long
        StrSQL = "Select FF_Sun From FF_Conf_List with(NOLOCK) Where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' Order By Convert(Decimal,FF_Sun) Desc "
        Re_Count = DB_Select(DBT)
        If Re_Count = 0 Then
            Db_Sun = 1
        Else
        ...
        '추가한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Insert into FF_Conf_List (FF_Code, FF_Sun)  Values ('" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "', '" & Db_Sun & "')"
        Re_Count = DB_Execute()
        Grid_Main_Display(Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value)
        Grid_Display_Ch = True
    End Sub
    ...
    Private Sub Save_Main_Click(sender As Object, e As EventArgs) Handles Save_Main.Click
        ...
        For i = 0 To Grid_Main.Rows.Count - 1
            If Grid_Main.Rows(i).HeaderCell.Value = "*" Then
                'Update
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "update FF_Conf_List Set FF_Name = '" & Grid_Main.Item(1, i).Value & "',  FF_Total = '" & Grid_Main.Item(2, i).Value & "', FF_Bigo = '" & Grid_Main.Item(3, i).Value & "' where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' And FF_Sun = '" & Grid_Main.Item(0, i).Value & "'"
                Re_Count = DB_Execute()
    ...
    Private Sub Delete__Main_Click(sender As Object, e As EventArgs) Handles Delete__Main.Click
        ...
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Delete FF_Conf_List Where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' And FF_Sun = '" & Grid_Main.Item(0, Grid_Main.CurrentCell.RowIndex).Value & "'"
        Re_Count = DB_Execute()

        For i = Grid_Main.CurrentCell.RowIndex + 1 To Grid_Main.RowCount - 1
            '수정한다
            StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
            StrSQL = StrSQL & "Update  FF_Conf_List Set FF_Sun = '" & i & "' Where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' And  FF_Sun = '" & i + 1 & "'"
            Re_Count = DB_Execute()
        Next i
        '선택된 갈럼값을 가지고 온다
        Grid_Main_Display(Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value)
    ...
    Private Sub Delete_Com_Click(sender As Object, e As EventArgs) Handles Delete_Com.Click
        ...
        If MsgBox("금형관리  '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "'를 삭제 하시겠습니까?", vbYesNo + vbQuestion + vbDefaultButton1, "금형관리 삭제") = vbNo Then
            Exit Sub
        End If
        ...
        '삭제한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Delete FF_Conf Where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' "
        Re_Count = DB_Execute()

        '삭제한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Delete FF_Conf_List Where FF_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "' "
        Re_Count = DB_Execute()
