...
Public Class Frm_CC_Report1
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(1).HeaderText = "수주일자"
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        If Search_Combo_Name.Text = "전체" And ComboBox2.Text = "전체" Then
            StrSQL = "Select CR_Date, CR_CM_NAME, CL_PL_Name, CL_Standard, CL_UNIT_AMOUNT, CL_QTY, CL_GOLD, CL_BIGO FROM SC_SALES with(NOLOCK), SC_SALES_LIST with(NOLOCK)  Where CL_CR_Code = CR_Code And CR_Date >= '" & S_Day.Text & "' And CR_Date <= '" & E_Day.Text & "' Order By CR_Date"

        ElseIf Search_Combo_Name.Text = "전체" And ComboBox2.Text <> "전체" Then
            StrSQL = "CR_Date, CR_CM_NAME, CL_PL_Name, CL_Standard, CL_UNIT_AMOUNT, CL_QTY, CL_GOLD, CL_BIGO  FROM SC_SALES with(NOLOCK), SC_SALES_LIST with(NOLOCK)  Where CL_CR_Code = CR_Code And CR_Date >= '" & S_Day.Text & "' And CR_Date <= '" & E_Day.Text & "' AND CL_PL_NAME ='" & ComboBox1.Text & "'  Order By CR_Date"

        ElseIf Search_Combo_Name.Text <> "전체" And ComboBox2.Text = "전체" Then
            StrSQL = "CR_Date, CR_CM_NAME, CL_PL_Name, CL_Standard, CL_UNIT_AMOUNT, CL_QTY, CL_GOLD, CL_BIGO  FROM SC_SALES with(NOLOCK), SC_SALES_LIST with(NOLOCK)  Where CL_CR_Code = CR_Code And CR_Date >= '" & S_Day.Text & "' And CR_Date <= '" & E_Day.Text & "' And  CR_CM_NAME = '" & Search_Combo.Text & "'  Order By CR_Date"
        Else
            StrSQL = "CR_Date, CR_CM_NAME, CL_PL_Name, CL_Standard, CL_UNIT_AMOUNT, CL_QTY, CL_GOLD, CL_BIGO  FROM SC_SALES with(NOLOCK), SC_SALES_LIST with(NOLOCK)  Where CL_CR_Code = CR_Code And CR_Date >= '" & S_Day.Text & "' And CR_Date <= '" & E_Day.Text & "' And  CR_CM_NAME = '" & Search_Combo.Text & "' AND CL_PL_NAME ='" & ComboBox1.Text & "'  Order By CR_Date"
        End If
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            ...
            For i = 0 To Re_Count - 1
                ...
                For j = 1 To Grid_Main.ColumnCount - 1
                    ...
                    If Grid_Main.Item(j, i).Value.ToString = DateTime.Today.ToString("yyyy-MM-dd") Then
                        Grid_Main.Rows(i).DefaultCellStyle.BackColor = Color.LightYellow
                    End If
                Next j
            Next i
        Else
