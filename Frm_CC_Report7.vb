...
Public Class Frm_CC_Report7
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(1).HeaderText = "코드번호"
        Grid_Main.Columns(2).HeaderText = "납품일자"
        Grid_Main.Columns(3).HeaderText = "납품시간"
        Grid_Main.Columns(4).HeaderText = "거래처명"
        Grid_Main.Columns(5).HeaderText = "제품코드"
        Grid_Main.Columns(6).HeaderText = "제품명"
        Grid_Main.Columns(7).HeaderText = "규격"
        Grid_Main.Columns(8).HeaderText = "단위"
        Grid_Main.Columns(9).HeaderText = "단가"
        Grid_Main.Columns(10).HeaderText = "Total"
        Grid_Main.Columns(11).HeaderText = "금액"
        Grid_Main.Columns(12).HeaderText = "VAT"
        Grid_Main.Columns(13).HeaderText = "비고"
        Grid_Main.Columns(14).HeaderText = "관리"
        Dim i As Integer
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        Grid_Main.RowCount = 0
        If Search_Combo_Name.Text = "전체" Then
            StrSQL = "Select SC_DELIVER.DL_Code,DL_Day,DL_Time,DL_Customer_Name,DL_PL_Code, DL_PL_Name, DL_Standard, DL_Unit, DL_Unit_Amount,
DL_Total, DL_Gold, DL_Vat, SC_DELIVER_LIST.DL_Bigo,  SC_DELIVER_LIST.DL_Management 
FROM SC_DELIVER JOIN SC_DELIVER_LIST ON SC_DELIVER.DL_Code=SC_DELIVER_LIST.DL_Code
where DL_Day >= '" & S_Day.Text & "' And DL_Day <= '" & E_Day.Text & "' Order By DL_Day"
        Else
            StrSQL = "Select SC_DELIVER.DL_Code,DL_Day,DL_Time,DL_Customer_Name,DL_PL_Code, DL_PL_Name, DL_Standard, DL_Unit, DL_Unit_Amount,
DL_Total, DL_Gold, DL_Vat, SC_DELIVER_LIST.DL_Bigo,  SC_DELIVER_LIST.DL_Management 
FROM SC_DELIVER JOIN SC_DELIVER_LIST ON SC_DELIVER.DL_Code=SC_DELIVER_LIST.DL_Code
where DL_Day >= '" & S_Day.Text & "' And DL_Day <= '" & E_Day.Text & "' And DL_Customer_Code = '" & Search_Combo.Text & "'  Order By DL_Day"
        End If
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
