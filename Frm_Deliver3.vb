...
Public Class Frm_Deliver3
    ...
    Private Sub Frm_Deliver_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Insert__Main.Visible = False
        Delete__Main.Visible = False
    ...
    Private Sub Insert_Com_Click(sender As Object, e As EventArgs) Handles Insert_Com.Click
        ...
        StrSQL = StrSQL & "Insert INTO SC_DELIVER (DL_Code, DL_Date, DL_Time, DL_Name, DL_Specifications, DL_Check ,DL_Management)  Values ('CP" & JP_Code & "', '" & My_Date & "','" & My_Time & "','" & Frm_Main.Text_Man_Name.Text & "','기본','','진행중' )"
        Re_Count = DB_Execute()
        Grid_Code_Display()
    ...
    Private Sub Grid_Code_CellEnter(sender As Object, e As DataGridViewCellEventArgs) Handles Grid_Code.CellEnter
        If Len(Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value) > 0 Then
            ...
            Grid_Info_Display2(Grid_Info, "Select * FROM SC_DELIVER With(NOLOCK) Where DL_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "'", Info_Txt, Info_Com)
    ...
    Public Function Grid_Main_Display(PL_Code As String) As Boolean
        ...
        StrSQL = "Select DL_Sun, DL_Day, DL_PL_Code, DL_PL_Name, DL_Standard, DL_Unit, DL_Unit_Amount, DL_Total, DL_Gold, DL_Vat, DL_Bigo,CL_Code,CL_Sun,DL_Management  FROM SC_DELIVER_LIST with(NOLOCK)  Where DL_Code = '" & PL_Code & "' Order By Convert(Decimal,DL_Sun)"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Private Sub Insert__Main_Click(sender As Object, e As EventArgs) Handles Insert__Main.Click
        ...
        '추가한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Insert INTO SC_DELIVER_LIST (DL_Code, DL_Sun, DL_Day, DL_Management)  Values ('" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "', '" & Db_Sun & "','" & My_Date & "' ,'" & "진행중" & "' )"
        Re_Count = DB_Execute()
        Grid_Main_Display(Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value)
        Grid_Display_Ch = True
    End Sub
    ...
    Private Sub Save_Main_Click(sender As Object, e As EventArgs) Handles Save_Main.Click
        ...
        Dim DL_Management As Integer
        For j = 0 To Grid_Main.Rows.Count - 1
            StrSQL = "select count(CL_Code) from SC_DELIVER_LIST where DL_PL_NAME='" & Grid_Main.Item(3, j).Value & "' AND CL_Code='" & Grid_Main.Item(11, j).Value & "' AND DL_Code='" & Grid_Info.Item(1, 0).Value & "'"
            Re_Count = DB_Select(Dbt)
            ...
            For i = 0 To Grid_Main.Rows.Count - 1
                If Grid_Main.Rows(i).HeaderCell.Value = "*" Then
                    'Update()
                    StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                    StrSQL = StrSQL & "UPDATE SC_DELIVER_LIST SET  DL_Management='" & Grid_Main.Item(13, i).Value & "'
        ...
        StrSQL = "select count(DL_Management) from SC_DELIVER_LIST where  DL_Code='" & Grid_Info.Item(1, 0).Value & "' AND DL_Management='완료'"
        Re_Count = DB_Select(Dbt)
        DL_Management = Dbt.Rows(0)(0).ToString()
        If (DL_Management = Grid_Main.Rows.Count) Then

            StrSQL = "UPDATE SC_DELIVER SET  DL_Management='완료'
        where DL_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "'"
            Re_Count = DB_Execute()
            MsgBox("납품 완료")
        Else
            StrSQL = "UPDATE SC_DELIVER SET  DL_Management='진행중'
        where DL_Code = '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "'"
            Re_Count = DB_Execute()
            MsgBox("납품 진행중")
    ...
    Private Sub Grid_Main_MouseClick(sender As Object, e As MouseEventArgs) Handles Grid_Main.MouseClick
        ...
        Select Case Grid_Main_Col
            ...
            Case 13
                Grid_Main_Combo.Items.Clear()
                Grid_Main_Combo.Items.Add("진행중")
                Grid_Main_Combo.Items.Add("완료")
                ...
                Next
                Grid_Main_Combo.Left = Grid_Main.Left + Grid_Main.RowHeadersWidth + Grid_Main.Columns(0).Width + Grid_Main.Columns(1).Width + Grid_Main.Columns(2).Width + Grid_Main.Columns(3).Width + Grid_Main.Columns(4).Width + Grid_Main.Columns(5).Width + Grid_Main.Columns(6).Width + Grid_Main.Columns(7).Width + Grid_Main.Columns(8).Width + Grid_Main.Columns(9).Width
                Grid_Main_Combo.Width = Grid_Main.Columns(Grid_Main_Col).Width
                Grid_Main_Combo.Text = Grid_Main.CurrentCell.Value.ToString
                Grid_Main_Combo.BackColor = Grid_Main.RowsDefaultCellStyle.BackColor
                Grid_Main_Combo.Visible = True
    ...
    Public Function DataGridView2_Display() As Boolean
        ...
        StrSQL = "select  SC_SALES_LIST.CL_Sun,SC_SALES_LIST.CL_Code , SC_SALES.CR_Date , SC_SALES.CR_Customer_Name  , SC_SALES_LIST.CL_PL_Name , SC_SALES_LIST.CL_Total ,SUM(convert(integer,SC_DELIVER_LIST.DL_Total)),'2'
                    FROM SC_SALES JOIN SC_SALES_LIST ON SC_SALES.CR_Code=SC_SALES_LIST.CL_Code LEFT OUTER JOIN SC_DELIVER_LIST ON SC_SALES_LIST.CL_Code=SC_DELIVER_LIST.CL_Code 
AND SC_SALES_LIST.CL_PL_Name=SC_DELIVER_LIST.DL_PL_Name where CL_Total!=DL_Total OR DL_Total IS NULL
 group by SC_SALES_LIST.CL_Sun,SC_SALES_LIST.CL_Code , SC_SALES.CR_Date , SC_SALES.CR_Customer_Name  , SC_SALES_LIST.CL_PL_Name 
    ...
    Public Function DataGridView1_Display() As Boolean
        ...
        StrSQL = "select  SC_SALES_LIST.CL_Code , SC_SALES.CR_Date , SC_SALES.CR_Customer_Name  , SC_SALES_LIST.CL_PL_Name , SC_SALES_LIST.CL_Total 
                    FROM SC_SALES JOIN SC_SALES_LIST ON SC_SALES.CR_Code=SC_SALES_LIST.CL_Code  where CR_Customer_Name='" + Grid_Info.Item(1, 5).Value + "'"
    ...
    Private Sub DataGridView1_CellMouseDoubleClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView1.CellMouseDoubleClick
        ...
        Info_Co8.Text = DataGridView1.Item(0, DataGridView1.CurrentCell.RowIndex).Value
        Grid_Info.Item(1, 8).Value = DataGridView1.Item(0, DataGridView1.CurrentCell.RowIndex).Value
        ...
        StrSQL = "Select CL_Sun, '" & DateTime.Now.ToString("yyyy-MM-dd") & "', CL_PL_Code, CL_PL_Name, CL_Standard, CL_Unit, CL_Unit_Amount, CL_Total, CL_Gold
                    FROM  SC_SALES_LIST
                    Where CL_PL_Name = '" & DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value & "' AND CL_Code='" & DataGridView1.Item(0, DataGridView1.CurrentCell.RowIndex).Value & "' Order By Convert(Decimal,CL_Sun)"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            Grid_Main.RowCount = Re_Count
            For i = 0 To Re_Count - 1
    ...
    Private Sub Grid_Main_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles Grid_Main.CellContentClick
        'Dim Dbt As New DataTable
        'StrSQL = "select CL_Code from SC_DELIVER_LIST where DL_Code='" & Grid_Info(1, 0).Value & "' AND DL_PL_Name='" & Grid_Main(3, Grid_Main.CurrentCell.RowIndex).Value & "'"
        'Re_Count = DB_Select(Dbt)
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        StrSQL = "select  SC_SALES_LIST.CL_Code , SC_SALES.CR_Date , SC_SALES.CR_Customer_Name  , SC_SALES_LIST.CL_PL_Name , SC_SALES_LIST.CL_Total 
                    FROM SC_SALES JOIN SC_SALES_LIST ON SC_SALES.CR_Code=SC_SALES_LIST.CL_Code  where CL_PL_Name Like'%" + TextBox2.Text + "%'"
