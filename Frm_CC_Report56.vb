...
Public Class Frm_CC_Report56
    ...
    Public Function S_Text_Setup() As Boolean
        ...
        StrSQL = "Select *  FROM SI_PRODUCT with(NOLOCK) Order By PL_Code"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            For i = 0 To Re_Count - 1
                Search_Combo.Items.Add(DBT.Rows(i)("PL_Code"))  'DBT.Rows(0)("PL_Name")
            Next i
        End If

        Search_Combo_Name.Items.Clear()
        Search_Combo_Name.Items.Add("전체")
        StrSQL = "Select *  FROM SI_PRODUCT with(NOLOCK) Order By PL_Name"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            For i = 0 To Re_Count - 1
                Search_Combo_Name.Items.Add(DBT.Rows(i)("PL_Name"))  'DBT.Rows(0)("PL_Name")
            Next i
        End If
    ...
    Private Sub Search_Combo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Search_Combo.SelectedIndexChanged
        ...
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            Search_Combo_Name.Text = DBT.Rows(0)("PL_Name")
        End If
    End Sub
    ...
    Private Sub Search_Combo_Name_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Search_Combo_Name.SelectedIndexChanged
        ...
        Dim DBT As New DataTable
        StrSQL = "Select *  FROM SI_PRODUCT with(NOLOCK) Where PL_Name = '" & Search_Combo_Name.Text & "'"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        Else
            StrSQL = "Select CL_PL_Code, CL_PL_Name, CL_Standard, Sum(Convert(Decimal,CL_Total)) As E1  , '' as E2, '' as E3, '' as E4 FROM SC_SALES_LIST with(NOLOCK), SC_SALES WITH(NOLOCK) Where CR_Code= SC_SALES_LIST.CL_Code and CL_PL_Code = '" & Search_Combo.Text & "' And  (CL_PL_CODE Is Not null Or Len(CL_PL_CODE) > 5) And Len(CL_PL_CODE) > 0  Group By CL_PL_Code, CL_PL_Name, CL_Standard, CL_Unit  Order By CL_PL_Code "
        End If
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
