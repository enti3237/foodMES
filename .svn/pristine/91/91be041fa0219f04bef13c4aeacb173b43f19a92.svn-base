...
Public Class Frm_Report2
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        Dim j As Integer
        Grid_Main.RowCount = 0
        StrSQL = "Select PO_PL_Code, PO_PL_Name, PO_Standard, PO_Unit, PO_PR_Gold, Sum(Convert(Decimal,PO_Total)) AS F1  FROM PC_ORDER with(NOLOCK), PC_ORDER_LIST with(NOLOCK), Product with(NOLOCK) Where PC_ORDER.PO_Code = PC_ORDER_LIST.PO_Code And PO_Date >= '" & S_Day.Text & "' And PO_Date <= '" & E_Day.Text & "' And Len(PO_Total) > 0 And Len(PO_DE_Code) > 0 And SI_Product.PL_Code = PO_PL_Code And PL_Sel = '제품'  "
