...
Module Main_Module
    ...
    Public Menu_String(22, 22) As String
    ...
    Public Function Count_Pro(Pro_Code As String, Go_Code As String, Go_C As String, To_C As String, Er_C As String) As Boolean
        ...
        PP_G_Sun = "0"
        PP_G_Amount = "0"
        StrSQL = "Select PP_Sun, PP_Amount FROM SI_Process_List with(NOLOCK) Where PP_Code = '" & Pro_Code & "' And PP_PC_Code = '" & Go_Code & "' "
        Re_Count = DB_Select(DBT)
        If Re_Count > 0 Then
        ...
        Else
            '변경
            StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
            StrSQL = StrSQL & "UPDATE SI_PROCESS_LIST Set PP_Amount = '" & Val(PP_G_Amount) + Val(To_C) & "' Where PP_Code = '" & Pro_Code & "' And PP_Sun = '" & PP_G_Sun & "'"
            Re_Count = DB_Execute()
            If PP_G_Sun = "1" Then
                '조합리스트의 마지막을 가지고 온다.
                ...
                If Re_Count > 0 Then
                    For i = 0 To Re_Count - 1
                        StrSQL = "Select  PP_Sun FROM SI_Process_List with(NOLOCK) Where PP_Code = '" & DBT.Rows(i)("PL_Sub_Code") & "' Order By Convert(Decimal,PP_Sun) Desc"
                        Re_Count2 = DB_Select(DBT2)
                        If Re_Count2 > 0 Then
                            StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                            StrSQL = StrSQL & "UPDATE SI_PROCESS_LIST Set PP_Amount =  Convert(Decimal,PP_Amount)  - " & Val(Go_C) * Val(DBT.Rows(i)("PL_Qty")) & "  Where PP_Code = '" & DBT.Rows(i)("PL_Sub_Code") & "' And PP_Sun = '" & DBT2.Rows(0)("PP_Sun") & "'"
                            Re_Count = DB_Execute()
                        End If
                    Next i
                End If
            Else
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "UPDATE SI_PROCESS_LIST Set PP_Amount = Convert(Decimal,PP_Amount)  - " & Val(Go_C) & " Where PP_Code = '" & Pro_Code & "' And PP_Sun = '" & Val(PP_G_Sun) - 1 & "'"
                Re_Count = DB_Execute()
            End If
        End If
