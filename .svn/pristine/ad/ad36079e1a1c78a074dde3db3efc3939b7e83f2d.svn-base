...
Public Class Frm_PC_Stock_Device_동영

    Dim D_Code As String
    Dim DD_Code As String
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        '        StrSQL = "Select PL_Go FROM SI_MACHINE_Ing with(NOLOCK) Where PL_Code = '" & D_Code & "' "
        '        Re_Count = DB_Select(DBT)
        '        If Re_Count <> 0 Then
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.RowTemplate.Height = 66
        ...
        Grid_Main.Columns(1).HeaderText = "작업지시전표"
        ...
        Grid_Main.Columns(8).HeaderText = "투입수량"
        Grid_Main.Columns(9).HeaderText = "생산수량"
        Grid_Main.Columns(10).HeaderText = "불량수량"
        ...
        Grid_Main.Columns(5).Width = 90
        Grid_Main.Columns(6).Width = 90
        Grid_Main.Columns(7).Width = 90

        For i = 8 To Grid_Main.ColumnCount - 1
            Grid_Main.Columns(i).Width = 70
        Next i
    ...
    Public Function Grid_Main_Display() As Boolean
        ...
        For i = 0 To Grid_Main.Rows.Count - 1
            StrSQL = "Select * FROM SI_MACHINE_Ing with(NOLOCK)  Where PL_PL_Code = '" & Grid_Main.Item(2, i).Value & "' And Len(PL_St_Day) > 0 And PL_Ing = 'ing'"
            Re_Count = DB_Select(DBT)
            If Re_Count <> 0 Then
                '시작시간
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        For i = 0 To Grid_Line_QC.Rows.Count - 1
            If Grid_Line_QC.Rows(i).HeaderCell.Value = "*" Then
                ...
                StrSQL = StrSQL & "update PO_Stock_Device_QC Set PS_Result = '" & Grid_Line_QC.Item(2, i).Value & "', PS_Bigo = '" & Grid_Line_QC.Item(3, i).Value & "' where PS_Device = '" & D_Code & "' And PS_Day = '" & Format(Now, "yyyy-MM-dd") & "' And PS_Sun = '" & Grid_Line_QC.Item(0, i).Value & "'"
                Re_Count = DB_Execute()
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        ...
        For i = 0 To Grid_Go.Rows.Count - 1
            If Grid_Go.Rows(i).HeaderCell.Value = "*" Then
                'Update
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "update PO_Stock_QC Set PQ_Result = '" & Grid_Go.Item(3, i).Value & "', PQ_Bigo = '" & Grid_Go.Item(4, i).Value & "' Where PQ_Code = '" & Grid_Main.Item(1, Grid_Main.CurrentCell.RowIndex).Value & "' And PQ_PL_Code = '" & Grid_Main.Item(2, Grid_Main.CurrentCell.RowIndex).Value & "' And PQ_PC_Code ='" & 공정코드 & "' And PQ_Sun = '" & Grid_Go.Item(0, i).Value & "'"
                Re_Count = DB_Execute()
    ...
    Private Sub Grid_Main_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles Grid_Main.MouseDoubleClick
        ...
        If Grid_Main.CurrentCell.ColumnIndex = 11 Then
            ...
            If Grid_Main.Item(11, Grid_Main.CurrentCell.RowIndex).Value = "시작" Then
                ...
                StrSQL = "Select PS_Sun, PS_History, PS_Result, PS_Bigo FROM PC_STOCK_Device_QC with(NOLOCK)  Where PS_Device = '" & DD_Code & "' And PS_Day = '" & Format(Now, "yyyy-MM-dd") & "' Order By Convert(Decimal,PS_Sun)"
                Re_Count = DB_Select(DBT)
                If Re_Count <> 0 Then
                Else
                    ...
                    Grid_Line_QC_Display(DD_Code)
                    Exit Sub
                End If
                'Select PQ_PC_Code, PQ_Sun, PQ_History1, PQ_History2,  PQ_Bigo  FROM SI_PRODUCT_QC with(NOLOCK) Where PQ_PL_Code = '300-100' And PQ_PC_Code = ''
                ...
                'DeVice_ing 처리 한다.
                'DeVice_ing 레코드가 있는가?
                'StrSQL = "Select *  FROM SI_MACHINE_Ing with(NOLOCK)  Where PL_Code = '" & D_Code & "'"
                'Re_Count = DB_Select(DBT)
                'If Re_Count <> 0 Then
                'Else
                '    '추가 한다.
                '    StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                '    StrSQL = StrSQL & "Insert INTO SI_MACHINE_Ing (PL_Code, PL_Name, PL_Group, PL_ID, PL_Port, PL_Sel, PL_Bigo)  Select * FROM SI_MACHINE with(NOLOCK) Where PL_Code = '" & D_Code & "' "
                '    Re_Count = DB_Execute()
                'End If
                '작업 지서서 번호
                ...
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "Insert into PC_Stock_List (PS_Code, PS_Sun, PS_PL_Code, PS_PL_Name, PS_Standard, PS_Unit, PS_Po_Total, PS_St_Day, PS_St_Time )   Select '" & 실적번호 & "', '1', PO_PL_Code, PO_PL_Name, PO_Standard, PO_Unit, PO_Total , '" & My_Date & "', '" & My_Time & "' FROM PC_ORDER_List  with(NOLOCK) Where PO_Code = '" & 지시서번호 & "' And  PO_PL_Code = '" & Grid_Main.Item(2, Grid_Main.CurrentCell.RowIndex).Value & "' "
                Re_Count = DB_Execute()
                ...
                'Update 공정코드 공정명
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "UPDATE SI_MACHINE_Ing Set PL_Ing = 'ing', PL_PS_Code = '" & 실적번호 & "', PL_PL_Code = '" & Grid_Main.Item(2, Grid_Main.CurrentCell.RowIndex).Value & "', PL_PL_Name = '" & Grid_Main.Item(3, Grid_Main.CurrentCell.RowIndex).Value & "', PL_PC_Code = '" & 공정코드 & "', PL_PC_Name = '" & 공정명 & "', PL_GO = '0', PL_Total = '0', PL_Er = '0'    where PL_Code = '" & DD_Code & "'"
                Re_Count = DB_Execute()
                StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                StrSQL = StrSQL & "UPDATE SI_MACHINE_Ing Set PL_St_Day = '" & My_Date & "', PL_St_Time = '" & My_Time & "',PL_En_Day = '', PL_En_Time = ''   where PL_Code = '" & DD_Code & "'"
                Re_Count = DB_Execute()
                ...
                '    StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
                '    StrSQL = StrSQL & "UPDATE SI_MACHINE_Ing Set PL_St_Day = '" & My_Date & "', PL_St_Time = '" & My_Time & "'   where PL_Code = '" & D_Code & "'"
                '    Re_Count = DB_Execute()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        StrSQL = "Select * FROM SI_MACHINE_Ing with(NOLOCK)  Where PL_Code = '" & DD_Code & "'"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
        ...
        '작업실적 전표 Update
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "update PO_Stock_List Set  PS_En_day = '" & My_Date & "', PS_En_Time = '" & My_Time & "', PS_Go = '" & Grid_Main.Item(8, Grid_Main.CurrentCell.RowIndex).Value & "', PS_Total = '" & Grid_Main.Item(9, Grid_Main.CurrentCell.RowIndex).Value & "', PS_Er = '" & Grid_Main.Item(10, Grid_Main.CurrentCell.RowIndex).Value & "' Where PS_Code = '" & NullStr & "' And PS_Sun = '1'"
        Re_Count = DB_Execute()
        ...
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "UPDATE SI_MACHINE_Ing Set PL_Ing = '', PL_En_Day = '" & My_Date & "', PL_En_Time = '" & My_Time & "'   where PL_Code = '" & D_Code & "'"
        Re_Count = DB_Execute()
