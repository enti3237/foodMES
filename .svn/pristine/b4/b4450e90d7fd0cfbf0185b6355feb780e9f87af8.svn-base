...
Public Class Frm_CC_Report4
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        Grid_Main.RowCount = 0
        If Search_Combo_Name.Text = "전체" Then
            StrSQL = "Select DL_Day, DL_PL_Code, DL_PL_Name, DL_Standard, DL_Unit, DL_Unit_Amount, DL_Total, DL_Gold, DL_Vat, SC_DELIVER_LIST.DL_Bigo  FROM SC_DELIVER with(NOLOCK), SC_DELIVER_LIST with(NOLOCK)  Where SC_DELIVER.DL_Code = SC_DELIVER_LIST.DL_Code And DL_Day >= '" & S_Day.Text & "' And DL_Day <= '" & E_Day.Text & "' Order By DL_Day"
        Else
            StrSQL = "Select DL_Day, DL_PL_Code, DL_PL_Name, DL_Standard, DL_Unit, DL_Unit_Amount, DL_Total, DL_Gold, DL_Vat, SC_DELIVER_LIST.DL_Bigo  FROM SC_DELIVER with(NOLOCK), SC_DELIVER_LIST with(NOLOCK)  Where SC_DELIVER.DL_Code = SC_DELIVER_LIST.DL_Code And DL_Day >= '" & S_Day.Text & "' And DL_Day <= '" & E_Day.Text & "' And DL_Customer_Code = '" & Search_Combo.Text & "'  Order By DL_Day"
        End If
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
