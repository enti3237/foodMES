...
Public Class Frm_PC_Report4
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        '재공재고 오리지널
        StrSQL = "Select DISTINCT CM_Stock.CS_Code,
                            CM_Stock_List.CS_PL_Code,
                            CM_Stock_List.CS_PL_Name,
                            CM_Stock_List.CS_Standard,
                            CM_Stock_List.CS_Unit,
                            CM_Stock.CS_Day,
                            SC_DELIVER_LIST.DL_Day,
                            SUM(CONVERT(INT, CM_Stock_List.CS_Total)) - SUM(CONVERT(INT, SC_DELIVER_LIST.DL_Total)) AS WorkIn
                  From CM_Stock With(NOLOCK), CM_Stock_List(NOLOCK)
                  LEFT OUTER JOIN Deliver_List with(NOLOCK) ON SC_DELIVER_LIST.DL_PL_Code = CM_Stock_List.CS_PL_Code
                  Where CM_Stock.CS_Code = CM_Stock_List.CS_Code 
                            and SC_DELIVER_LIST.DL_Day >= '" & S_Day.Text & "'
                            and SC_DELIVER_LIST.DL_Day <= '" & E_Day.Text & "' 
                            and CM_Stock_List.CS_PL_Code = 
		                        ALL	(	select PL_Sub_Code
				                        FROM SI_PRODUCT_RECIPE 
				                        where CM_Stock_List.CS_PL_Code = Product_BOM.PL_Sub_Code 
			                        )
                                        GROUP BY CM_Stock.CS_Code, CM_Stock_List.CS_PL_Code, CM_Stock_List.CS_PL_Name, CM_Stock_List.CS_Standard, CM_Stock_List.CS_Unit, CM_Stock.CS_Day, SC_DELIVER_LIST.DL_Day
                  "
