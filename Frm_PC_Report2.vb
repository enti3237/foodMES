...
Public Class Frm_PC_Report2
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(0).HeaderText = "거래처명"
        Grid_Main.Columns(1).HeaderText = "제품명"
        Grid_Main.Columns(2).HeaderText = "규격"
        Grid_Main.Columns(3).HeaderText = "지시수량"
        Grid_Main.Columns(4).HeaderText = "생산시작일자"
        Grid_Main.Columns(5).HeaderText = "생산종료일자"
        Grid_Main.Columns(6).HeaderText = "생산수량"
        Grid_Main.Columns(7).HeaderText = "불량수량"
        Grid_Main.Columns(8).HeaderText = "공정검사여부"
    ...
    Public Function Grid_Main_Display() As Boolean
        ...
        '현황 보기
        StrSQL = "Select top 100 CR_CM_Name, PO_PL_Name, PO_Standard, PS_PO_Total, PS_St_Day, PS_En_Day, PS_Total, PS_Er, Check_G, PS_Bigo
                        FROM PC_Stock with(NOLOCK)
                        JOIN PC_Order
                        ON PC_Stock.PS_PO_Code = PC_Order.PO_Code 
						join SC_SALES
						on SC_SALES.CR_Code = PC_Order.PO_CR_CODE
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        If Search_Combo.Text = "제품명" Then
            StrSQL = "Select PL_Code, PL_Name FROM SI_PRODUCT with(NOLOCK) "
            StrSQL = StrSQL & " WHERE PL_SEL='제품' Order By PL_Code"
        Else
            StrSQL = "Select PL_Code, PL_Name FROM SI_PRODUCT with(NOLOCK) WHERE PL_CODE='" & Search_Txt.Text & "'"
            StrSQL = StrSQL & " AND PL_SEL='제품' Order By PL_Code"
        End If
        ...
        If Search_Txt.Text = "전체" Then
            ...
            For i = 0 To Grid_Main.RowCount - 1
                ...
                StrSQL = StrSQL & " Where PC_ORDER.PO_Code = PC_ORDER_LIST.PO_Code And PO_Date >= '" & S_Day.Text & "' And PO_Date <= '" & E_Day.Text & "' And Len(PO_Total) > 0 And Len(PO_PL_CODE) > 0 And PO_PL_CODE = '" & Grid_Main.Item(1, i).Value & "'"
                Re_Count = DB_Select(DBT)
    ...
    Private Sub Search_Combo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Search_Combo.SelectedIndexChanged
        ...
        Dim DBT As New DataTable
        StrSQL = "Select *  FROM SI_PRODUCT with(NOLOCK) Where PL_Code = '" & Search_Combo.Text & "' Order By PL_Code"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            Search_Txt.Text = DBT.Rows(0)("PL_Name")
        End If
    End Sub

    Private Sub Search_Combo_Name_SelectedIndexChanged(sender As Object, e As EventArgs)
        If Search_Txt.Text = "전체" Then
        ...
        Dim DBT As New DataTable
        StrSQL = "Select *  FROM SI_PRODUCT with(NOLOCK) Where PL_Name = '" & Search_Txt.Text & "'"
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
            Search_Combo.Text = DBT.Rows(0)("PL_Code")
        End If
    End Sub
