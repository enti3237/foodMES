...
Public Class Frm_Facilities_con
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(4).HeaderText = "소모품코드"
        Grid_Main.Columns(5).HeaderText = "소모품명"
    ...
    Private Sub Insert_Com_Click(sender As Object, e As EventArgs) Handles Insert_Com.Click
        ...
        StrSQL = StrSQL & "Insert into FC_RECORD (FL_Code, FL_Date, FL_Time, FL_Name)  Values ('FL" & JP_Code & "', '" & My_Date & "','" & My_Time & "','" & Frm_Main.Text_Man_Name.Text & "')"
        Re_Count = DB_Execute()
        Grid_Code_Display()
    ...
    Public Function Grid_Code_Display() As Boolean
        ...
        Select Case Search_Combo.Text
            Case "코드"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Code Like '%" & Search_Text.Text & "%'  Order  By FL_Code DESC"
            Case "날짜"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Date Like '%" & Search_Text.Text & "%'  Or FL_Date Is Null Order  By FL_Date Desc"
            Case "담당자"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Name Like '%" & Search_Text.Text & "%'   Order  By FL_Name Desc"
        End Select
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Private Sub Insert__Main_Click(sender As Object, e As EventArgs)
        ...
        '추가한다
        StrSQL = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED "
        StrSQL = StrSQL & "Insert into FC_RECORD_List (FL_Code, FL_Sun)  Values ('" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "', '" & Db_Sun & "')"
        Re_Count = DB_Execute()
    ...
    Private Sub Grid_Main_MouseClick(sender As Object, e As MouseEventArgs) Handles Grid_Main.MouseClick
        ...
        Select Case Grid_Main_Col
            ...
            Case 4
                StrSQL = "Select PL_Code, PL_Name  FROM SI_PRODUCT with(NOLOCK) where PL_Sel = '소모품' Order By PL_Code"
                Re_Count = DB_Select(DBT)
                Grid_Main_Combo.Items.Clear()
                If Re_Count <> 0 Then
                    For i = 0 To Re_Count - 1
                ...
                Grid_Main_Combo.Focus.ToString()
            Case 5
                StrSQL = "Select PL_Code, PL_Name  FROM SI_PRODUCT with(NOLOCK) where PL_Sel = '소모품' Order By PL_Name"
                Re_Count = DB_Select(DBT)
                Grid_Main_Combo.Items.Clear()
                If Re_Count <> 0 Then
                    For i = 0 To Re_Count - 1
    ...
    Private Sub Grid_Main_Combo_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles Grid_Main_Combo.SelectionChangeCommitted
        ...
        Select Case Grid_Main_Col
            ...
            Case 4
                StrSQL = "Select PL_Code, PL_Name FROM SI_PRODUCT with(NOLOCK) Where PL_Code = '" & Grid_Main_Combo.SelectedItem.ToString() & "' "
                Re_Count = DB_Select(DBT)
                If Re_Count <> 0 Then
            ...
            Case 5
                StrSQL = "Select PL_Code, PL_Name FROM SI_PRODUCT with(NOLOCK) Where PL_Name = '" & Grid_Main_Combo.SelectedItem.ToString() & "' "
                Re_Count = DB_Select(DBT)
                If Re_Count <> 0 Then
