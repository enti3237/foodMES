...
Public Class Frm_Facilities2
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(9).Visible = False
    ...
    Private Sub Insert_Com_Click(sender As Object, e As EventArgs) Handles Insert_Com.Click
        ...
        StrSQL = StrSQL & "Insert into FC_RECORD (FL_Code, FL_Date, FL_Time, FL_Name,FL_Sel)  Values ('FL" & JP_Code & "', '" & My_Date & "','" & My_Time & "','" & Frm_Main.Text_Man_Name.Text & "','설비계획')"
        Re_Count = DB_Execute()
        Grid_Code_Display()
    ...
    Public Function Grid_Code_Display() As Boolean
        ...
        Select Case Search_Combo.Text
            Case "코드"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Code Like '%" & Search_Text.Text & "%' and FL_SEL='설비계획'  Order  By FL_Code DESC"
            Case "날짜"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Date Like '%" & Search_Text.Text & "%'  Or  FL_Date Is Null AND FL_SEL='설비계획' Order  By FL_Date Desc"
            Case "담당자"
                StrSQL = "Select FL_Code, FL_Date, FL_Name From FC_RECORD with(NOLOCK) Where FL_Name Like '%" & Search_Text.Text & "%' AND FL_SEL='설비계획'  Order  By FL_Name Desc"
        End Select
        Re_Count = DB_Select(DBT)
        If Re_Count <> 0 Then
    ...
    Private Sub Delete_Com_Click(sender As Object, e As EventArgs) Handles Delete_Com.Click
        ...
        If MsgBox("설비계획  '" & Grid_Code.Item(0, Grid_Code.CurrentCell.RowIndex).Value & "'를 삭제 하시겠습니까?", vbYesNo + vbQuestion + vbDefaultButton1, "설비계획 삭제") = vbNo Then
            Exit Sub
        End If
