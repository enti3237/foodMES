...
Public Class Frm_QC_Report2
    ...
    Public Function Grid_Main_Setup() As Boolean
        ...
        Grid_Main.Columns(1).HeaderText = "생산수량"
        Grid_Main.Columns(2).HeaderText = "불량유형"
        Grid_Main.Columns(3).HeaderText = "불량수량"
        Grid_Main.Columns(4).HeaderText = "불량율"
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        '         Select PS_Date, PS_History, Sum(Convert(Decimal,PS_Total)) AS F1 FROM PC_STOCK_Er with(NOLOCK), PO_Stock with(NOLOCK) Where   PC_Stock.PS_Code = PO_Stock_Er.PS_Code And   PS_Date + PS_Time >= '2019-04-0100:00' And PS_Date + PS_Time <= '2019-04-1123:59'  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date
        StrSQL = "Select PS_Date, Sum(Convert(Decimal,PS_Go)) AS F1 , PS_History, Sum(Convert(Decimal,PO_Stock_Er.PS_Total)) AS F2 FROM PC_STOCK_Er with(NOLOCK), PO_Stock with(NOLOCK) , PC_Stock_List with(NOLOCK) Where   PC_Stock.PS_Code = PO_Stock_Er.PS_Code And PC_Stock.PS_Code = PC_STOCK_LIST.PS_Code And  PS_Date + PS_Time >= '" & S_Day.Text & S_Time.Text & "' And PS_Date + PS_Time <= '" & E_Day.Text & E_Time.Text & "'  And Len(PO_Stock_Er.PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date "
        'Select PS_Date, PS_History, Sum(Convert(Decimal,PS_Total)) AS F1 FROM PC_STOCK_Er with(NOLOCK), PO_Stock with(NOLOCK) Where   PC_Stock.PS_Code = PO_Stock_Er.PS_Code And  And PS_Date + PS_Time >= '2019-04-0100:00' And PS_Date + PS_Time <= '2019-04-1123:59'  And Len(PS_Total) > 0 GROUP BY PS_Date, PS_History  Order By PS_Date 
        ...
        If Re_Count <> 0 Then
            ...
            For i = 0 To Re_Count - 1
                ...
                Next j
                Grid_Main.Item(4, i).Value = Int(Val(Grid_Main.Item(3, i).Value) / Val(Grid_Main.Item(1, i).Value) * 100) / 100
            Next i
        Else
            Grid_Main.RowCount = 1
        ...
        '    StrSQL = "Select Sum(Convert(Decimal,PP_Amount)) AS F1 FROM SI_Process_List with(NOLOCK) "
