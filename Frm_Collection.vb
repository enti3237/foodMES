Public Class Frm_Collection
    ...
    Private Sub Search_Com_Click(sender As Object, e As EventArgs) Handles Search_Com.Click
        ...
        StrSQL = "Select DL_Date, DL_Customer_Name, sum(convert(decimal, DL_Gold)) AS DL_Total_Gold, SC_DELIVER.DL_Bigo
                  FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock) 
                  where DL_Date >= '" & Day1.Text & "' and DL_Date <='" & Day2.Text & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
        ...
        If RadioButton1.Checked Then
            ...
            If Day1.Text = date2 Then
                ...
                StrSQL = "Select DL_Date, DL_Customer_Name, sum(convert(decimal, DL_Gold)) AS DL_Total_Gold, SC_DELIVER.DL_Bigo
                      FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock) 
                      where DL_Date >= '" & date2 & "' and DL_Date <='" & Day2.Text & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
                Re_Count = DB_Select(DBT)
                If Re_Count <> 0 Then
                    For i = 0 To Re_Count - 1
            ...
            Else
                ...
                For k = 0 To ysub
                    ...
                    StrSQL = "Select DL_Date, DL_Customer_Name, sum(convert(decimal, DL_Gold)) AS DL_Total_Gold, SC_DELIVER.DL_Bigo
                      FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock) 
                      where DL_Date >= '" & d1.ToString & "' and DL_Date <='" & date1 & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
            ...
            'StrSQL = "Select DL_Date, DL_Customer_Name, sum(convert(decimal, DL_Gold)) AS DL_Total_Gold, SC_DELIVER.DL_Bigo
            '      FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock) 
            '      where DL_Date >= '" & Day1.Text & "' and DL_Date <='" & date1 & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
            'Re_Count = DB_Select(DBT)
            'If Re_Count <> 0 Then
            '    For i = 0 To Re_Count - 1
            '        DL_Total_Gold += DBT.Rows(i)("DL_Total_Gold")
        ...
        ElseIf RadioButton2.Checked Then
            ...
            Else
                ...
                For k = 0 To ysub
                    ...
                    StrSQL = "Select DL_Date, DL_Customer_Name, sum(convert(decimal, DL_Gold)) AS DL_Total_Gold, SC_DELIVER.DL_Bigo
                      FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock) 
                      where DL_Date >= '" & dd & "' and DL_Date <='" & date1 & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
    ...
    Private Sub Customer_Grid_Display(code1 As String)
        ...
        StrSQL = "Select DL_Day, DL_PL_Name, DL_PL_Code, DL_Total, DL.DL_Bigo
                  FROM SC_DELIVER_LIST DL with(NOLOCK), Deliver with(nolock)
                  where DL_Date = '" & code1 & "' and DL.DL_Code= SC_DELIVER.DL_Code "
        'where DL_Date >= '" & Day1.Text & "' and DL_Date <='" & Day2.Text & "' and DL.DL_Code=SC_DELIVER.DL_Code and DL_Gold !='' and DL_Name!='' group by DL_Date, DL_Customer_Name, DL_PL_Code, SC_DELIVER.DL_Bigo"
